...
func collectPLCData(key string) {
	// Simula leitura de dados do PLC via rede.
	simulatedValues := []map[string]interface{}{
		{
			"plc_id":      1,
			"register_id": 1001,
			"value_float": 42.5,
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to marshal PLC payload for %s: %v\n", key, err)
		return
	}

	// TODO: Publicar `message` em uma fila (NATS/Redis) no tópico configurado, ex.: "plc.data".
	fmt.Fprintf(os.Stderr, "publish plc data message: %s\n", string(message))

	// TODO: Conectar ao SQL e executar: UPDATE plcs SET is_online = true, last_seen = NOW() WHERE key = p.key
}
